document.addEventListener('DOMContentLoaded', function () {
    ...
    const blueBar = document.querySelector('.navbar'); // Select the blue bar element
    const leftIcons = document.querySelector('.left-icons');
    let sidebarVisible = false; // Track sidebar visibility

    blueBar.addEventListener('click', function () {
        sidebarVisible = !sidebarVisible; // Toggle visibility state
        if (sidebarVisible) {
            leftIcons.classList.add('show'); // Show sidebar on click
        } else {
            leftIcons.classList.remove('show'); // Hide sidebar on click again
        }
    });

    // Sidebar content toggle functionality for each icon
